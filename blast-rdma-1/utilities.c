...
our_print_statistics(struct our_control *conn, struct our_options *options)
{
	...
	fprintf(stderr, "%s: %lu rdma_write completions\n",
			options->message, conn->wc_rdma_write);

	if (options->flags & PRINT_STATS) {
		if (dlapse > 0.0 && conn->wc_rdma_write > 0) {
			fprintf(stderr, "%s: %.6f Messages per second\n",
				options->message,
				((double)(conn->wc_rdma_write))/dlapse);
			fprintf(stderr, "%s: %.6f Megabits per second\n",
				options->message,
				8.0 * ((double)((conn->wc_rdma_write)
				* options->data_size)) / (dlapse * 1000000.0));
		}
		if (conn->wc_rdma_write > 0) {
			fprintf(stderr,"%s: %.3f microseconds per message blasted\n",
				options->message,
				1000000.0 * dlapse/((double)conn->wc_rdma_write));
		}
	}
