...
#define OUR_DEFAULT_LIMIT	70000
...
#define OUR_DEFAULT_N_DATA_BUFFERS	4
 ...
 * we need 1 for each user_data buffer we allocate
 */
#define OUR_MAX_WORK_REQUESTS	OUR_MAX_USER_DATA_BUFFERS

/* maximum number of user sge's referenced in any one work request */
#define OUR_MAX_SGES		1

/* maximum number of user send_queue and recv_queue elements */
#define OUR_SQ_DEPTH	OUR_MAX_WORK_REQUESTS
#define OUR_RQ_DEPTH	OUR_MAX_WORK_REQUESTS
...
/***** this part contains fields for CM and completions interaction *****/
	...
	/* list of linked send work requests ready to blast */
	struct ibv_send_wr		*work_request_list_head;
	struct ibv_send_wr		*work_request_list_tail;
	unsigned int			work_request_list_size;
...
/* fields added to get statistics on completion event handling */
	 ...
	 * size determined by max_n_work_completions
	 * histogram printed as "work_completion_array_size[i] occurrences k"
	 * weighted_total printed as "total work completions j"
	 */
...
/***** this part contains fields for data setup and transfer *****/
	...
	/* each points to one dynamically allocated user_data buffers */
	unsigned char		*user_data[OUR_MAX_USER_DATA_BUFFERS];

	/* each points to one memory registration item for one user_data buffer */
	struct ibv_mr		*user_data_mr[OUR_MAX_USER_DATA_BUFFERS];

	/* each holds metadata for one user_data buffer
	 * and is referenced by one user_data work request
	 */
	struct ibv_sge		user_data_sge[OUR_MAX_USER_DATA_BUFFERS];

	/* each holds work request for one user_data send operation */
	struct ibv_send_wr	user_data_send_work_request[OUR_MAX_USER_DATA_BUFFERS];

	/* each holds work request for one user_data recv operation */
	struct ibv_recv_wr	user_data_recv_work_request[OUR_MAX_USER_DATA_BUFFERS];

	/* number of user_data buffers actually allocated and registered in
	 * user_data and user_data_mr arrays by our_setup_user_data()
	 * - valid only after call to our_setup_user_data()
	 * - used to know how many to eventually deregister and deallocate
	 */
 ...
 */
    ...
    */
	/* holds metainfo we send to remote about our local data buffers */
	struct our_buffer_info		local_buffer_info[OUR_MAX_USER_DATA_BUFFERS];

	/* points to memory registration item for local_buffer_info itself */
	struct ibv_mr			*local_buffer_info_mr[OUR_MAX_SGES];

	/* work request for one send operation */
	struct ibv_send_wr		local_buffer_info_work_request;

	/* holds metadata for local_buffer_info
	 * and is referenced by local_buffer_info_work request
	 */
    ...
    */
	/* holds metainfo we recv about remote data buffers */
	struct our_buffer_info		remote_buffer_info[OUR_MAX_USER_DATA_BUFFERS];

	/* points to memory registration item for remote_buffer_info itself */
	...
	/* holds metadata for remote_buffer_info
	 * and is referenced by remote_buffer_info_work request
	 */
	struct ibv_sge			remote_buffer_info_sge[OUR_MAX_SGES];

	/* counts number of successful RDMA_WRITE calls to ibv_post_send() */
	unsigned long			send_stats;

	/* array to hold histogram of number of chained RDMA_WRITE work requests
	 * posted on each call to ibv_post_send()
	 * size determined by OUR_MAX_WORK_REQUESTS
	 * histogram printed as "work_request_list_size[i] occurrences k"
	 * weighted_total printed as "total work requests j"
	 */
...
extern void
our_add_to_work_request_list(struct our_control *conn,
		struct ibv_send_wr *work_request, struct our_options *options);


/* this builds new conn->work_request_list_head from work completion array
 *
 * on return conn->work_request_list_head will be non-NULL if all ok, else NULL
 */
extern int
our_get_work_request_list(struct our_control *conn, struct our_options *options);
