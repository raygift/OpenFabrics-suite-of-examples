		  ...
		  struct our_options *options)
{
	...
	if (n_user_bufs <= 0) {
		fprintf(stderr, "%s: n_user_bufs %d not positive\n",
			options->message, n_user_bufs);
		ret = ENOMEM;
		goto out1;
	}
	...
	our_trace_uint64("our_setup_user_data", "n_user_data_bufs",
						conn->n_user_data_bufs,options);
	ret = 0;
	goto out0;
out1:
			...
			int n_remote_sges, struct our_options *options)
{
	...
	/* fill in each local_buffer_info element with metainfo about one
	 * dynamically allocated local user_data buffer
	 */
 ...
 *	n_data_buffers to hold the original data client will RDMA_WRITE
 *
 * returns == 0 if all ok,
 *	   != 0 on error (and error message has been given)
*/
...
our_setup_client_buffers(struct our_control *conn, struct our_options *options)
{
	...
	/* client needs multiple buffers, all to RDMA_WRITE blast data out of */
	ret = our_setup_user_data(conn, options->n_data_buffers, access, options);
	if (ret != 0) {
		goto out0;
	}

	/* client needs n_user_data_bufs work requests, because
	 * each RDMA_WRITE blasts data out of its own user_data[i]
	 * link all these work requests into 1 list for use in 1 ibv_post_send()
	 */
	conn->work_request_list_head = NULL;
	conn->work_request_list_size = 0;
	for (i = 0; i < conn->n_user_data_bufs; i++) {
		our_setup_send_wr(conn, &conn->user_data_sge[i],
					IBV_WR_RDMA_WRITE, 1,
					&conn->user_data_send_work_request[i]);
		/* add new work request to end of work_request_list */
		our_add_to_work_request_list(conn,
				&conn->user_data_send_work_request[i], options);
	}
 ...
 *	n_data_buffers to recv() the client's RDMA_WRITE data
 *
 * returns == 0 if all ok,
 *	   != 0 on error (and error message has been given)
*/
...
our_setup_agent_buffers(struct our_control *conn, struct our_options *options)
{
	...
	/* agent needs multiple buffers for remote client to RDMA_WRITE into */
	ret = our_setup_user_data(conn, options->n_data_buffers, access, options);
	if (ret != 0) {
		goto out0;
	}
