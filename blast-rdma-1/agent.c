/* agent.c - client uses rdma_write to blast user data */
/*
...
our_agent_operation(struct our_control *agent_conn, struct our_options *options)
{
	...
	expected_size = agent_conn->n_user_data_bufs
	...
	agent_conn->wc_rdma_write = ntohl(agent_conn->recv_ack.ack_count);
