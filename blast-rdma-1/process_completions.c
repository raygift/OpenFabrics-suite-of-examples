...
{
	if (conn->work_request_list_head == NULL) {
		/* first item being added to empty list */
		conn->work_request_list_head = work_request;
	} else {
		conn->work_request_list_tail->next = work_request;
	}
	conn->work_request_list_tail = work_request;
	work_request->next = NULL;
	conn->work_request_list_size += 1;
...
our_get_work_request_list(struct our_control *conn, struct our_options *options)
{
	...
	/* we now have at least 1 unused work completion in our array
	 * form a list of all corresponding work requests
	 */
	while (conn->index_work_completions < conn->current_n_work_completions) {
		work_completion
			=&conn->work_completion[conn->index_work_completions++];
		ret = our_check_completion_status(conn,work_completion,options);
		if (ret != 0) {
			/* don't allow any more work requests to be posted */
			conn->work_request_list_head = NULL;
			goto out0;
		}
		if (work_completion->opcode & IBV_WC_RECV) {
			fprintf(stderr, "%s: unexpected IBV_WC_RECV\n",
							options->message);
			/* don't allow any more work requests to be posted */
			conn->work_request_list_head = NULL;
			ret = -1;
		...
		/* count this completed RDMA_WRITE */
		conn->wc_rdma_write++;

		/* see if we need to post this work request again */
		if (conn->n_posted_buffers + conn->work_request_list_size
							 < options->limit) {
			/* point at work request that generated this completion*/
			work_request
				 = (struct ibv_send_wr *)work_completion->wr_id;
		
			/* add this send work request to end of linked list */
			our_add_to_work_request_list(conn, work_request, options);
		}
	}	/* while */
out0:
	return ret;
}	/* our_get_work_request_list */
