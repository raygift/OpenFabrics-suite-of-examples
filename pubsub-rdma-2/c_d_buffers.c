...
{
	int	ret;
	int	access[OUR_MAX_USER_DATA_BUFFERS] =
			{IBV_ACCESS_LOCAL_WRITE |
			IBV_ACCESS_REMOTE_WRITE, 0, 0};

	/* client needs 3 buffers:
	 * user_data[0] for remote agent to RDMA_WRITE published data into
	 * user_data[1] to capture temporary copy of current user_data[0]
	 * (since user_data[0] may change while we are looking at it)
	 * user_data[2] holds latest (different) value of published data
	 * (since successive published values may be identical, we need this
	 * so we only print changed values)
	 */
	ret = our_setup_user_data(conn,OUR_MAX_USER_DATA_BUFFERS,access,options);
	if (ret != 0) {
		goto out0;
	}
	our_trace_ok("client's setup_user_data", options);

	/* client needs no work request, because agent does all the work */
...
{
	int	ret;
	int	access[1] = {IBV_ACCESS_REMOTE_READ};

	/* agent needs 1 buffer:
	 * user_data[0] into which the agent generates the publish data
	 * and then RDMA_WRITEs out of it to the client subscriber
	 */
	...
	/* agent needs 1 work requests:
	 * one to RDMA_WRITE publish data out of user_data[0]
	 */
