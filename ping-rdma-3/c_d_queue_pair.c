		...
		struct our_options *options)
{
	...
	our_report_ulong("our_create_cq", "max_n_work_completions",
...
our_destroy_cq(struct our_control *conn, struct our_options *options)
{
	...
	/* acknowledge all accumulated successful ibv_get_cq_event() calls */
	if (conn->cq_events_that_need_ack != 0) {
		ibv_ack_cq_events(conn->completion_queue,
			...
			struct our_options *options)
{
	...
	if (conn->completion_channel == NULL) {
		...
		ret = 0;
		our_trace_ptr("ibv_create_comp_channel",
				"created completion channel",
				conn->completion_channel, options);
		our_trace_ulong("ibv_create_comp_channel",
				"assigned fd",
