/* client.c -- using only rdma_write to ping-pong user data */
/*
 ...
 *	clear pong buffer to all zeros
 *	post an RDMA_WRITE send to data to the agent
 *	wait for outstanding send (RDMA_WRITE) to complete
 *	spin testing pong buffer to become all non-zeros
 *
 * The client breaks out of the loop on any error, including the FLUSH_ERR,
 * which means the agent has disconnected.
 *
 * returns == 0 if all ok,
