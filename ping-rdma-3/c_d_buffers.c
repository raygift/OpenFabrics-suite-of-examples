 ...
 * for each buffer
 *	allocates space for user_data, registers it
		  ...
		  struct our_options *options)
{
	...
	for (i = 0; i < n_user_bufs; i++) {
		...
		/* setup each user_data buffer for appropriate access */
...
{
	int	ret;
	int	access[2] = {	IBV_ACCESS_LOCAL_WRITE |
				IBV_ACCESS_REMOTE_WRITE,

				IBV_ACCESS_REMOTE_READ};

	/* client needs 2 buffers,
	 * user_data[1] for local client to RDMA_WRITE ping data out of
	 * user_data[0] for remote agent to RDMA_WRITE pong data into
	 */
...
our_setup_agent_buffers(struct our_control *conn, struct our_options *options)
{
	...
	/* agent needs 2 buffers
	 * user_data[0] for remote client to RDMA_WRITE ping data into
	 * user_data[1] for local agent to RDMA_WRITE pong data out of
	 */
	ret = our_setup_user_data(conn, 2, access, options);
	if (ret != 0) {
		goto out0;
	 ...
	 * has to be filled in dynamically
	 */
