...
our_setup_client_buffers(struct our_control *conn, struct our_options *options)
{
	 ...
	 * user_data[0] for local client to RDMA_READ published data into
	 * user_data[1] holds latest (different) value of published data
	...
	/* client needs 1 work request:
	 * one to RDMA_READ published data into user_data[0]
	 */
	our_setup_send_wr(conn, &conn->user_data_sge[0], IBV_WR_RDMA_READ, 1,
					&conn->user_data_send_work_request[0]);

	/* the remote info fields of the RDMA_READ work request
...
our_setup_agent_buffers(struct our_control *conn, struct our_options *options)
{
	 ...
	 * and from which the client subscriber will read it
	 */
