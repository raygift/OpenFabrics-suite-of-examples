	  ...
	  struct our_options *listener_options)
{
	...
	/* post all n_data_buffers recv work requests on the local agent_conn
	 * so remote client can start blasting right after the connect
	 */
	for (i = 0; i < options.n_data_buffers; i++) {
		if (our_post_recv(agent_conn,
			&agent_conn->user_data_recv_work_request[i],
			&options) != 0)
		goto out4;
	}
